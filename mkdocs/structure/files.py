            ...
            if path not in self.src_uris:
                for dir in config.theme.dirs:
                    ...
                    if os.path.isfile(os.path.join(dir, path)):
                        self.append(File(config, path, src_dir=dir, asset_versioning='hash_rename'))
                        break
...
class File:
    ...
    def __init__(
        ...
        asset_versioning: Literal['none', 'hash_rename', 'hash_suffix'] = 'none',
    ):
        self.page = None
        self.src_path = path
    ...
    def z(
        ...
        *,
        ...
        asset_versioning: Optional[Literal['none', 'hash_rename', 'hash_suffix']] = None,
    ):
        if src_dir is None:
        ...
        if asset_versioning is None:
            if any(fnmatch.fnmatch(path, pat) for pat in config['asset_patterns']):
                asset_versioning = config.asset_versioning
            else:
                asset_versioning = 'none'
        self.__init__(path, src_dir, dest_dir, use_directory_urls, asset_versioning)
